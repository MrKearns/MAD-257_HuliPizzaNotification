...
//            self.introNotification()
            ...
            content.title = "Scheduled a Pizza"
            ...
            self.addNotification(trigger: trigger, content: content, identifer: identifier)
        }
    }
...
//            self.introNotification()
    ...
    func addNotification(trigger: UNNotificationTrigger, content:UNMutableNotificationContent, identifer:String){
        let request = UNNotificationRequest(identifier: identifer, content: content, trigger: trigger)
        
        UNUserNotificationCenter.current().add(request) { (error) in self.printError(error, location: "Add request for identifier" + identifer)
